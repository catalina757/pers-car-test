...
module.exports = (function() {
  ...
  Swag.addHelper('lengthBigger', function(array, length, options) {
    if(!length){
      length = 0;
    }
    if (!Utils.isUndefined(length)) {
  ...
  Number.prototype.format1 = function (n, x, s, c) {
    let re = '\\d(?=(\\d{' + (x || 3) + '})+' + (n > 0 ? '\\D' : '$') + ')',
      num = this.toFixed(Math.max(0, ~~n));
    return (c ? num.replace('.', c) : num).replace(new RegExp(re, 'g'), '$&' + (s || ','));
  };

  Swag.addHelper('currency', function (v, decimals) {
    v = Number(v);
    return v != null && !isNaN(v) ? v.format1(decimals ? decimals : 0, 3, '.', ',') : null;
  });
  ...
  Dates.fullMonths = ['Ianuarie', 'Februarie', 'Martie', 'Aprilie', 'Mai', 'Iunie', 'Iulie', 'August', 'Septembrie', 'Octombrie', 'Noviembrie', 'Decembrie'];
  ...
  Swag.addHelper('datePoint', d=> {
    if(d){
      d = new Date(d);
      return Dates.format(d, '%d.%m.%Y');
    }
    return null;
  });

  Swag.addHelper('dateMonthYear', d=> {
    if(d){
      d = new Date(d);
      return Dates.format(d, '%B %y');
    }
    return null;
  });

  Swag.addHelper('square', function(val) {
    return '<div class="square">'+(val ? 'X' : '&nbsp')+'</div>';
  });
  ...
  Swag.addHelper('addPoint', function(txt, lng) {
    if(!txt){
      txt = '.'.repeat(lng);
    }
    return txt;
  ...
  Swag.addHelper('addChar', function(txt, char, lngEmpty, lngNotEmpty, bold) {
    if(!txt){
      txt = char.repeat(lngEmpty);
    }else{
      if(bold){
        txt = '<b>'+txt+'</b>';
      }
      if(lngNotEmpty && lngNotEmpty-(txt.length/1.75).round() > 0){
        txt += char.repeat(lngNotEmpty-(txt.length/1.75).round());
      }
    }
  ...
  Swag.addHelper('squareCheck', function(val, checkOnTrue) {
    return !checkOnTrue ? '<span class="no-indent"><div class="square">'+(!!val ? '&nbsp' : '<b>X</b>')+'</div></span>' : '<span class="no-indent"><div class="square">'+(!!val ? '<b>X</b>' : '&nbsp')+'</div></span>';
  });
